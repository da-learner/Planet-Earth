...
class hotels {
	...
	function hotels() {
		...
		add_action('manage_hotels_posts_columns',array($this,'columns'),10,2);
		add_action('manage_hotels_posts_custom_column',array($this,'column_data'),11,2);
	...
	function create_taxonomies() {
		...
		register_taxonomy('hotels_type',array('hotels'),$args);
		...
		register_taxonomy('hotels_attribute','hotels',$args);
	}

	function columns($columns) {
		unset($columns['date']);
		unset($columns['taxonomy-hotels_attribute']);
	...
	function join($wp_join) {
		global $wpdb;
		if(get_query_var('post_type') == 'hotels') {
			$wp_join .= " LEFT JOIN (
