...
class holidays {
	...
	function holidays() {
		...
		add_action('manage_holidays_posts_columns',array($this,'columns'),10,2);
		add_action('manage_holidays_posts_custom_column',array($this,'column_data'),11,2);
	...
	function create_taxonomies() {
		...
		register_taxonomy('holidays_type',array('holidays'),$args);
		...
		register_taxonomy('holidays_attribute','holidays',$args);
	}

	function columns($columns) {
		unset($columns['date']);
		unset($columns['taxonomy-holidays_attribute']);
	...
	function join($wp_join) {
		global $wpdb;
		if(get_query_var('post_type') == 'holidays') {
			$wp_join .= " LEFT JOIN (
